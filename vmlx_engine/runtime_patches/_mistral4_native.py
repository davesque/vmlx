...
class Mistral4Attention(nn.Module):
    ...
    def __call__(
        ...
        if L == 1:
            ...
            k = v = kv_latent
            # vMLX/JANG fast fix (mirrors deepseek_v32.py): MLA absorb path at
            # bf16 drifts ~7.0 in logit magnitude per token vs the prefill path
            # because the SDPA kernel contracts over the 512-dim kv_lora_rank
            # in bf16 (7-bit mantissa). Cast SDPA inputs to float32 ONLY on
            # the L==1 branch to preserve precision; tensors are tiny so the
            # cost is negligible (~1-3%, no measurable speed regression). Cast
            # output back to bf16 before unembed_out. ONLY the L==1 (decode)
            # branch is touched — prefill is byte-identical to upstream.
